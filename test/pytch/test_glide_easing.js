...
describe("Glide easing", () => {
    ...
    it("computes piecewise-quadratic", async () => {
        const project = await import_deindented(`
        ...
        const exp_ts = [...Array(23).keys()].map(t => t / 22.0).map(t =>
            ((t < 0.5)
             ? (2.0 * t * t)
             : (1.0 - 0.5 * (-2 * t + 2) * (-2 * t + 2))));
        ...
        assert.strictEqual(got_ts.length, exp_ts.length);
        got_ts.forEach((got_t, idx) => {
            const exp_t = exp_ts[idx];
            assert_float_close(got_t, exp_t, 1.0e-15);
        });
    });
});
